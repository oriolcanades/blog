---
layout: post
title:  "Controladores: Validaciones de entrada"
date:   2024-03-02
categories: [java, spring-boot, controllers, data-validation]
---

# Controladores: Validaciones de entrada
_10/02/2024 - v1.0.0 - Oriol Canadés_
...
Para comenzar a utilizar JSR-303 en tu proyecto Spring Boot, el primer paso es incluir la dependencia necesaria en tu archivo pom.xml (Maven) o  build.gradle (Grdale). Esto habilitará la funcionalidad de validación en tu proyecto:
...
Para usar una validación en un controlador, simplemente lo pasas como argumento en un método del controlador anotado con @Valid o @Validated. Esto activará la validación automática de Spring al recibir una solicitud, y si hay errores de validación, estos se pueden manejar o reportar adecuadamente.
...
En este ejemplo, @NotBlank se asegura de que el campo name no esté vacío, @Email valida que el campo email contenga una dirección de correo electrónico válida, y @Min verifica que el campo age sea un número positivo.
